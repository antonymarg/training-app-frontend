...
import { AddTrainingPageContainer } from './addEditTrainingPage.style';
import { AutocompleteUserMultiple, DateTimePicker } from '../';
import { eTrainingTopics, eTrainingTypes } from '../../lib/enums';
import { Timestamp } from 'firebase/firestore';
import {
  IAddEditTrainingForm,
  IAddEditTrainingFormErrors,
} from './addEditTrainingPage.types';

const TOPIC_LIST = Object.keys(eTrainingTopics) as Array<
  keyof typeof eTrainingTopics
>;

export interface IAddEditTrainingPageProps {
  formData: IAddEditTrainingForm;
  errors: IAddEditTrainingFormErrors;
  handleInputChange: (input: Partial<IAddEditTrainingForm>) => void;
  onContinue: () => Promise<void>;
  isLoading: boolean;
  title: string;
  buttonLabel: string;
}
export function AddEditTrainingPage({
  formData,
  errors,
  handleInputChange,
  onContinue,
  ...
  return (
    <AddTrainingPageContainer>
      ...
      <Button
        ...
        size="large"
        onClick={onContinue}
        style={{ gridArea: 'button', width: 'fit-content', marginLeft: 'auto' }}
      >
        {isLoading ? (
